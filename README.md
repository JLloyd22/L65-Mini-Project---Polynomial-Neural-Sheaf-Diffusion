...
2. Run a shell with GPU access (adjust volume paths for your system):
   ```bash
   podman run -it --rm \
     -v /home/$USER/PolySheafNeuralNetworks-PowerGrids:/work/project \
...
3. Inside the container, launch sweeps as usual:
   ```bash
   wandb sweep --project PolySheafNeuralNetworks-PowerGrids sweeps/nc/dblp/diag_sheaf.yaml
   wandb agent sheaf_hypergraphs/PolySheafNeuralNetworks-PowerGrids/<SWEEP_ID>
   ```
For mac, instead: 
```
