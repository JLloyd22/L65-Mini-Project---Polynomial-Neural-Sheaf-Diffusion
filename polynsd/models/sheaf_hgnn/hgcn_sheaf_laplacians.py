...
def reduce_graph(X_reduced, m, d, edge_index):
    ...
    def key_func_diag(x):
        return x[1]  # this type sort according to the nodes
...
def SheafLaplacianDiag(H, m, d, edge_index, sheaf, E=None):
    ...
    H_selected = torch.index_select(
        H_selected, dim=0, index=edge_index[0]
    )  # select nodes corresponding to the pairs in nnz
    X_reduced = H_selected.permute(0, 2, 1)  # nnz x f x d
    sheaf = torch.diag_embed(sheaf, dim1=-2, dim2=-1)  # nnz x d x d

    sheaf = (
        sheaf.unsqueeze(1).repeat(1, MLP_hidden, 1, 1).reshape(-1, d, d)
    )  # nnz f x d x d
    ...
    )  # combine non-diagonal and diagonal elements of the laplacian
...
def SheafLaplacianGeneral(H, m, d, edge_index, sheaf, E=None):
    ...
    sheaf = (
        sheaf.unsqueeze(1).repeat(1, MLP_hidden, 1, 1).view(-1, d, d)
    )  # nnz f x d x d
    ...
    F = F.view(
        F.shape[0], d, d
    )  # nnz x d x d, all the linear projections for all (node, edge) pairs
    F_source = torch.index_select(F, dim=0, index=edges_idx[0])  # F_v<e(X_v)
    F_dest = torch.index_select(F, dim=0, index=edges_idx[1])  # F_v<e(X_v)
    attributes = -1 * torch.bmm(
        F_source.transpose(1, 2), F_dest
...
def SheafLaplacianOrtho(H, m, d, edge_index, sheaf, E=None):
    ...
    sheaf = sheaf.view(
        sheaf.shape[0], d, d
    )  # nnz x d x d containinf all the projections for F_v<e
    sheaf = (
        sheaf.unsqueeze(1).repeat(1, MLP_hidden, 1, 1).view(-1, d, d)
    )  # nnz x f x d x d
