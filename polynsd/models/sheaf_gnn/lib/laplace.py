...
def sym_matrix_pow(matrix: torch.Tensor, p: float) -> torch.Tensor:
    r"""Power of a matrix using Eigen Decomposition.
...
def build_sheaf_difussion_matrix(
    N, K, edge_index, maps, augmented=True, return_laplacian=False
):
    ...
    """
    L_index, L_val = build_norm_sheaf_laplacian(
        N, K, edge_index, maps, augmented=augmented
    )
...
def compute_learnable_laplacian_indices(size, edge_index, learned_d, total_d):
    ...
    row_template = (
        torch.arange(0, learned_d, device=device).view(1, -1, 1).tile(1, 1, learned_d)
    )
    col_template = torch.transpose(row_template, dim0=1, dim1=2)

    non_diag_row_indices = (row_template + total_d * row.reshape(-1, 1, 1)).reshape(
        1, -1
    )
    non_diag_col_indices = (col_template + total_d * col.reshape(-1, 1, 1)).reshape(
        1, -1
    )
