...
from polynsd.models.hgnn_baselines.mlp import MLP
...
class PMA(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # This is the encoder part. Where we use 1 layer NN (Theta*x_i in the GATConv
        # description) Now, no seed as input. Directly learn the importance weights
        # alpha_ij. self.lin_O = Linear(heads*self.hidden, self.hidden) # For heads
        # combining For neighbor nodes (source side, key)
        self.lin_K = Linear(
            in_channels, self.heads * self.hidden, weight_initializer="glorot"
        )
