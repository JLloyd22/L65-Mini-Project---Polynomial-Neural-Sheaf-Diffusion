...
from utils.powergrid_loader import load_powergrid_dataset, load_powergrid_node_dataset, load_unified_powergrid_node_dataset
...
def get_dataset(name: str):
    ...
    # --- Power grid datasets (local .mat files) ---
    elif name_key in {"ieee24", "ieee39", "ieee118", "uk"}:
        dataset = load_powergrid_dataset(name_key, root=data_root, label_type="bi")

    # --- Power grid node-level datasets (snapshots) ---
    elif name_key in {"ieee24_node", "ieee39_node", "ieee118_node", "uk_node"}:
        dataset = load_powergrid_node_dataset(name_key, root=data_root, target="Y_polar")

    # --- Unified Power grid node-level dataset ---
    elif name_key in {"all_grids_node", "unified_node"}:
        dataset = load_unified_powergrid_node_dataset(root=data_root, target="Y_polar")
