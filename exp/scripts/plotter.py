...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Plot restriction maps stored in restriction_maps.pt or U-matrix .pt files in a directory.")
    parser.add_argument(
        "--umap-file",
        type=Path,
        default=None,
        help="If set, plot this single U-matrix .pt file as a heatmap and exit.",
    )
    parser.add_argument(
        "checkpoint_dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="If set, plot all .pt U-matrix files in this directory as heatmaps (bypasses restriction_maps.pt logic)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main():
    ...
    args = parse_args()
    # If --umap-file is set, plot just that file as a heatmap and exit
    if args.umap_file is not None:
        pt_file = args.umap_file
        out_dir = args.output_dir or pt_file.parent
        out_dir = Path(out_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        u_matrix = torch.load(pt_file, map_location="cpu")
        if hasattr(u_matrix, "numpy"):
            u_matrix = u_matrix.numpy()
        import os
        ...
        plt.xlabel('Feature')
        plt.ylabel('Node')
        plt.title(f'U-matrix: {os.path.basename(pt_file)}')
        plt.tight_layout()
        out_path = out_dir / f"{os.path.basename(pt_file)}_plot.png"
        plt.savefig(out_path)
        plt.show()
        plt.close()
        print(f"Saved U-matrix heatmap: {out_path}")
        return
    # If --umap-dir is set, plot all .pt files in that directory as heatmaps and exit
    if args.umap_dir is not None:
        umaps_dir = args.umap_dir
        out_dir = args.output_dir or umaps_dir
        out_dir = Path(out_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        for fname in os.listdir(umaps_dir):
            if fname.endswith(".pt"):
                u_matrix = torch.load(umaps_dir / fname, map_location="cpu")
                if hasattr(u_matrix, "numpy"):
                    u_matrix = u_matrix.numpy()
                plt.figure(figsize=(10, 4))
                plt.imshow(u_matrix, aspect='auto', cmap='viridis')
                ...
                plt.xlabel('Feature')
                plt.ylabel('Node')
                plt.title(f'U-matrix: {fname}')
                plt.tight_layout()
                plt.savefig(out_dir / f"{fname}_plot.png")
                plt.close()
                print(f"Saved U-matrix heatmap: {out_dir / f'{fname}_plot.png'}")
        return
