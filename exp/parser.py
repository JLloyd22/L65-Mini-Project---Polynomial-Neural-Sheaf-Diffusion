...
def get_parser():
    ...
    parser.add_argument('--model', type=str, choices=[
        ...
        # Discrete polynomial
        'DiagSheafPolynomial', 'BundleSheafPolynomial', 'GeneralSheafPolynomial', 'PolySpectralGNN',
        # ODE variants
    ...
    # ---------- Snapshot dataset controls ----------
    parser.add_argument('--snapshot_train_ratio', type=float, default=0.85,
                        help="Fraction of snapshots for training (paper uses 0.85).")
    parser.add_argument('--snapshot_val_ratio', type=float, default=0.05,
                        help="Fraction of snapshots for validation (paper uses 0.05).")
    parser.add_argument('--snapshot_train_samples_per_epoch', type=int, default=0,
                        help="If >0, subsample this many training snapshots per epoch. "
                             "0 means use all training snapshots (recommended).")
    parser.add_argument('--snapshot_eval_samples', type=int, default=0,
                        help="If >0, subsample this many snapshots for val/test evaluation. "
                             "0 means use all val/test snapshots (recommended).")
    parser.add_argument('--batch_size', type=int, default=32,
                        help="Number of snapshots per training batch (paper uses 32).")
    parser.add_argument('--edge_feat_dim', type=int, default=0,
                        help="Edge feature dimension. Auto-set from dataset if edge_attr.mat exists. "
                             "Override to 0 to disable edge features even if the file is present.")

    # ---------- Polynomial filter args (generalized) ----------
    parser.add_argument("--lambda_max_choice", choices=["analytic", "iterative"], default="analytic",
                        help="How to estimate/upper-bound lambda_max for polynomial spectral filters.")
    ...
    parser.add_argument("--gegenbauer_lambda", type=float, default=1.0,
                        help="lambda parameter for Gegenbauer (must be > 0).")
    parser.add_argument("--jacobi_alpha", type=float, default=0.0,
                        help="alpha parameter for Jacobi (must be > -1).")
    parser.add_argument("--jacobi_beta", type=float, default=0.0,
                        help="beta parameter for Jacobi (must be > -1).")
    ...
    parser.add_argument("--rotation_invariant_sheaf_learner", type=str2bool, default=False,
                        help="Use the RotationInvariantSheafLearner (O(d) maps).")
    parser.add_argument("--node_edge_sims_time_dependent", type=str2bool, default=False,
                        help="Make node-edge similarity features time-dependent for the rotation-invariant learner.")
